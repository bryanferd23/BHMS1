<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <a href=" {{ route('payment.create') }} " class="px-3 py-1 text-xs text-white bg-gray-800 rounded-lg">
                + ADD
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <form class="max-w-md mx-auto mb-4" method="GET" action="{{ route('payment.search') }}">
                @csrf
                        ...
                        @foreach ($payments as $payment)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <th scope="row" class="px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                                    {{$payment->boarder->name}}
                                </th>
                                <th scope="row" class="px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                                    {{$payment->boarder->email}}
                                </th>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex space-x-4">
                                        <div>
                                            <a href="{{ route('payment.edit', $payment->id) }}" class="bg-gray-800 hover:bg-gray-900 text-white text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-900">Edit</a>
                                        </div>
                                        <div>
