...
<div class="py-12">
    <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
        ...
        <form class="max-w-md mx-auto mb-4" method="GET" action="{{ route('balance.search') }}">
            @csrf
                ...
                <tbody>
                    @foreach ($boarders as $boarder)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td class="px-6 py-4">
                                {{'P '.number_format($boarder->total)}}
                            </td>
                            <td class="
                            @if ($boarder->balance < 0)
                            text-blue-600
                            @elseif ($boarder->balance < 3000 && $boarder->balance >= 1000)
                            text-yellow-500
                            @elseif ($boarder->balance >= 3000)
                            text-red-600
                            text-bold
                            @else
                            text-green-600
                            @endif
                                px-6 py-4">
                                {{$boarder->balance > 0 ? 'P '.number_format($boarder->balance) : 'Advance: ' . number_format($boarder->balance/-100) . ' day/s'}}
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex space-x-4">
                                    <div>
                                        <a href="{{ route('payment.search') }}?search={{$boarder->name}}" class="bg-gray-800 hover:bg-gray-900 text-white text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-900">View</a>
                                    </div>
                                </div>
                            </td>
                        </tr>
